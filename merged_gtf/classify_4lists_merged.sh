...
echo "[1/6] Koordynaty transkryptow (z gene_id)..."
cat "$GTF" | awk '$3=="transcript" {
    gene = $10; gsub(/"/, "", gene); gsub(/;.*/, "", gene)
    print $1"\t"($4-1)"\t"$5"\t"gene
}' > "$TMPDIR/transcripts_gene_full.bed"

echo "[2/6] Koordynaty eksonow (z gene_id)..."
cat "$GTF" | awk '$3=="exon" {
    gene = $10; gsub(/"/, "", gene); gsub(/;.*/, "", gene)
    print $1"\t"($4-1)"\t"$5"\t"gene
}' > "$TMPDIR/exons_gene_full.bed"

echo "[3/6] Filtrowanie: usuwanie retrogenow z adnotacji (zeby nie nachodzily same na siebie)..."
cut -f1-3 "$INPUT" > "$TMPDIR/retrocopy_mask.bed"
# -f 0.9 AND -F 0.9 = obustronne 90%: element GTF-a ~ identyczny z retrogenem
# (usuwa retrogeny same siebie, ale nie usuwa genow-gospodarzy ani malych eksonow)
bedtools intersect -a "$TMPDIR/transcripts_gene_full.bed" -b "$TMPDIR/retrocopy_mask.bed" -v -f 0.9 -F 0.9 > "$TMPDIR/transcripts_gene.bed"
bedtools intersect -a "$TMPDIR/exons_gene_full.bed" -b "$TMPDIR/retrocopy_mask.bed" -v -f 0.9 -F 0.9 > "$TMPDIR/exons_gene.bed"
echo "  -> Usunieto $(($(wc -l < "$TMPDIR/transcripts_gene_full.bed") - $(wc -l < "$TMPDIR/transcripts_gene.bed"))) transkryptow bedacych retrogenami"
echo "  -> Usunieto $(($(wc -l < "$TMPDIR/exons_gene_full.bed") - $(wc -l < "$TMPDIR/exons_gene.bed"))) eksonow bedacych retrogenami"

echo "[4/6] Koordynaty transkryptow (bez gene_id, do klasyfikacji)..."
cut -f1-3 "$TMPDIR/transcripts_gene.bed" > "$TMPDIR/transcripts.bed"

echo "[5/6] Koordynaty eksonow (bez gene_id, do klasyfikacji)..."
cut -f1-3 "$TMPDIR/exons_gene.bed" > "$TMPDIR/exons.bed"

echo "[6/6] Filtrowanie single_cds_merged.bed..."
bedtools intersect -a single_cds_merged.bed -b "$TMPDIR/retrocopy_mask.bed" -v -f 0.9 -F 0.9 > "$TMPDIR/single_cds_filtered.bed"

if [ ! -f single_cds_merged.bed ]; then
    echo "Uruchamiam CDS_one_exon_merged.py..."
    python3 CDS_one_exon_merged.py
fi
# Uzyj odfiltrowanej wersji (bez retrogenow)
CDS_BED="$TMPDIR/single_cds_filtered.bed"
...
add_gene_ids() {
    ...
    local annot_bed="$2"
    local output="$3"
    local overlap="${4:-$MIN_OVERLAP}"   # domyslnie 0.5, ale mozna podac inny
...
bedtools intersect -a "$INPUT" -b "$CDS_BED" -wa -u -f "$MIN_OVERLAP" | sort_bed > "$TMPDIR/list3_classified.bed"
...
if [ -s "$TMPDIR/list3_classified.bed" ]; then
    bedtools intersect -a "$TMPDIR/list3_classified.bed" -b "$CDS_BED" -wa -wb -f "$MIN_OVERLAP" \
        | awk '
        {
...
# Nizszy prog dla eksonow (0.1) bo retrogen moze byc na granicy ekson-intron
bedtools intersect -a "$INPUT" -b "$TMPDIR/exons.bed" -wa -u -f 0.1 | sort_bed > "$TMPDIR/exon_hits.bed"
...
add_gene_ids "$TMPDIR/list4_classified.bed" "$TMPDIR/exons_gene.bed" "$TMPDIR/list4_with_gene.bed" 0.1
...
# --- KOPIUJ WYNIKI ---
cp "$TMPDIR/list1_with_gene.bed" list1_intergenic_merged.bed
cp "$TMPDIR/list2_with_gene.bed" list2_intronic_merged.bed
cp "$TMPDIR/list3_with_gene.bed" list3_cds_one_exon_merged.bed
cp "$TMPDIR/list4_with_gene.bed" list4_exonic_multigene_merged.bed
...
declare -A totals
for f in list1_intergenic_merged.bed list2_intronic_merged.bed list3_cds_one_exon_merged.bed list4_exonic_multigene_merged.bed; do
    count=$(cut -f4 "$f" 2>/dev/null | sort -u | wc -l)
    printf "%-35s %8d\n" "$f" "$count"
    totals["$f"]=$count
done

sum=$(( totals[list1_intergenic_merged.bed] + totals[list2_intronic_merged.bed] + totals[list3_cds_one_exon_merged.bed] + totals[list4_exonic_multigene_merged.bed] ))
echo "----------------------------------------"
printf "%-35s %8d\n" "SUMA (4 listy)" "$sum"
...
echo ""
echo "Pliki wynikowe:"
echo "  list1_intergenic_merged.bed        - międzygenowe"
echo "  list2_intronic_merged.bed          - intronowe"
echo "  list3_cds_one_exon_merged.bed      - CDS w 1 egzonie"
echo "  list4_exonic_multigene_merged.bed  - eksonowe (geny wielo-egzonowe)"
echo ""
