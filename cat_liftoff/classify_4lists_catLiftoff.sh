...
bedtools intersect -a "$TMPDIR/transcripts_gene_full.bed" -b "$TMPDIR/retrocopy_mask.bed" -v -f 0.9 > "$TMPDIR/transcripts_gene.bed"
bedtools intersect -a "$TMPDIR/exons_gene_full.bed" -b "$TMPDIR/retrocopy_mask.bed" -v -f 0.9 > "$TMPDIR/exons_gene.bed"
...
echo "[6/6] Filtrowanie single_cds.bed..."
bedtools intersect -a single_cds.bed -b "$TMPDIR/retrocopy_mask.bed" -v -f 0.9 > "$TMPDIR/single_cds_filtered.bed"

if [ ! -f single_cds_cat.bed ]; then
    echo "Uruchamiam CDS_one_exon_cat.py..."
    python3 CDS_one_exon_cat.py
fi
...
# --- KOPIUJ WYNIKI ---
cp "$TMPDIR/list1_with_gene.bed" list1_intergenic_cat.bed
cp "$TMPDIR/list2_with_gene.bed" list2_intronic_cat.bed
cp "$TMPDIR/list3_with_gene.bed" list3_cds_one_exon_cat.bed
cp "$TMPDIR/list4_with_gene.bed" list4_exonic_multigene_cat.bed
...
declare -A totals
for f in list1_intergenic_cat.bed list2_intronic_cat.bed list3_cds_one_exon_cat.bed list4_exonic_multigene_cat.bed; do
    count=$(cut -f4 "$f" 2>/dev/null | sort -u | wc -l)
    printf "%-35s %8d\n" "$f" "$count"
    totals["$f"]=$count
done

sum=$(( totals[list1_intergenic_cat.bed] + totals[list2_intronic_cat.bed] + totals[list3_cds_one_exon_cat.bed] + totals[list4_exonic_multigene_cat.bed] ))
echo "----------------------------------------"
printf "%-35s %8d\n" "SUMA (4 listy)" "$sum"
...
echo ""
echo "Pliki wynikowe:"
echo "  list1_intergenic_cat.bed        - międzygenowe"
echo "  list2_intronic_cat.bed          - intronowe"
echo "  list3_cds_one_exon_cat.bed      - CDS w 1 egzonie"
echo "  list4_exonic_multigene_cat.bed  - eksonowe (geny wielo-egzonowe)"
echo ""
